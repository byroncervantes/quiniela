...
<div class="space-y-8">
    ...
    <!-- Welcome Header -->
    <div class="p-6 md:p-8 rounded-3xl mariscal-gradient text-white shadow-xl shadow-blue-900/10 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
        <!-- Abstract light graphic backgrounds -->
        <div class="absolute right-0 top-0 w-64 h-64 bg-white/5 rounded-full blur-3xl pointer-events-none -mr-16 -mt-16"></div>
        ...
        <div class="relative z-10 space-y-2">
            ...
            <p class="text-blue-100 max-w-xl text-sm font-medium leading-relaxed">
                Te damos la bienvenida a <strong>QuinMariscal</strong>, la quiniela oficial de Distribuidora Mariscal. Prepárate para predecir los marcadores del Mundial FIFA 2026 y competir con tus compañeros de trabajo.
            </p>
        ...
        <div class="relative z-10 flex flex-col sm:flex-row gap-3">
            <a href="{{ route('matches') }}" class="px-5 py-3 rounded-xl bg-white text-blue-900 font-bold text-sm text-center shadow-md hover:bg-slate-100 transition-all active:scale-95">
    ...
    <div class="space-y-4">
        <h2 class="text-xl font-bold text-[#0f2240] flex items-center gap-2">
            <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5a2 2 0 10-2 2h2zm0 0L4 12m8 0l8-4L12 20"/></svg>
                    ...
                    <div class="glass-card rounded-3xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between">
                        <div class="p-6 space-y-4">
                            ...
                            <div class="flex justify-between items-start gap-4">
                                <div>
                                    <h3 class="text-lg font-bold text-[#0f2240]">{{ $pool->name }}</h3>
                                    <p class="text-xs text-slate-400 font-semibold uppercase tracking-wider">{{ $pool->tournament->name }}</p>
                                </div>
                            ...
                            @if($participant->status === 'approved')
                                ...
                                <div class="grid grid-cols-4 gap-2 pt-2 text-center">
                                    <div class="p-2.5 rounded-2xl bg-blue-50 border border-blue-100/50">
                                    ...
                                    <div class="p-2.5 rounded-2xl bg-amber-50 border border-amber-100/50">
                                        <span class="block text-2xl font-extrabold text-amber-600">{{ $participant->exact_scores_count }}</span>
                                        <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Exacto</span>
                                    </div>
                                    <div class="p-2.5 rounded-2xl bg-emerald-50 border border-emerald-100/50">
                                        <span class="block text-2xl font-extrabold text-emerald-600">{{ $participant->correct_results_count }}</span>
                                    ...
                                    <div class="p-2.5 rounded-2xl bg-slate-50 border border-slate-100">
                                        <span class="block text-2xl font-extrabold text-slate-700">
                        ...
                        <div class="px-6 py-4 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between gap-4">
                            @if($participant->status === 'approved')
                                <a href="{{ route('pools.predictions', $pool->slug) }}" class="flex-1 px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs text-center shadow-sm transition-colors cursor-pointer">
                                    Pronosticar Partidos
                                </a>
                                <a href="{{ route('pools.ranking', $pool->slug) }}" class="flex-1 px-4 py-2.5 rounded-xl bg-slate-200 hover:bg-slate-300 text-[#0f2240] font-bold text-xs text-center transition-colors cursor-pointer">
                                    Tabla de Posiciones
                                </a>
                            @else
                                <div class="w-full text-center text-xs text-amber-600 font-semibold bg-amber-50 rounded-xl py-2 px-3">
    ...
    @if($availablePools->isNotEmpty())
        <div class="space-y-4 pt-4">
            <h2 class="text-xl font-bold text-[#0f2240] flex items-center gap-2">
                <svg class="w-6 h-6 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/></svg>
                Otras Quinielas Disponibles
            </h2>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                @foreach($availablePools as $pool)
                    <div class="glass-card rounded-3xl border border-slate-100 overflow-hidden shadow-sm flex flex-col justify-between">
                            ...
                            @if($pool->join_mode === 'invitation_code')
                                <form action="{{ route('pools.join', $pool->id) }}" method="POST" class="flex gap-2">
                                    ...
                                    <input type="text" name="invitation_code" required placeholder="Código..." 
                                        class="flex-grow px-3.5 py-2 rounded-xl bg-white border border-slate-200 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500">
                                    <button type="submit" class="px-4 py-2 rounded-xl bg-blue-600 text-white font-bold text-xs hover:bg-blue-500 cursor-pointer">
                                        Unirse
                                    </button>
                                </form>
                            @else
                                <form action="{{ route('pools.join', $pool->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="w-full px-4 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs text-center transition-colors cursor-pointer">
